...
Aprovisiona un Function App que aloja las dos funciones del flujo de confirmación y notificación de pagos (`ValidatePayment`, HTTP trigger; `ProcessPayment`, Service Bus trigger). Deliberadamente separado del Service Plan Dedicated que usa `compute-appservice`.

**Se instancia dos veces** (`instance_suffix = ""` y `"-canary"`, raíz de cada ambiente) — `func-novapay-pagos-${env}` y `func-novapay-pagos-canary-${env}`, dos recursos fijos sin jerarquía permanente: cuál sirve tráfico vigente lo decide el peso del backend pool de APIM en cada momento, no el nombre del recurso (ADR-03, U4, ver `novapay-iac-terraform`/`decisiones/`). Cada instancia tiene plan, storage account y Subscription de Service Bus **dedicados** — nunca compartidos entre sí.
...
- `instance_suffix` (`""` estable / `"-canary"` candidata) y `storage_account_name` (explícito, no derivado — el límite de 24 caracteres de Storage Account exige un nombre abreviado propio para canary).
- `integracion_subnet_id` (de `networking`, subred delegada a `Microsoft.Web/serverFarms`), `max_instance_count` (sin default — obliga a decidir explícitamente por ambiente, ver más abajo), `servicebus_namespace_fqdn` / `servicebus_topic_name` / `servicebus_subscription_name` (de `messaging-servicebus` — Topic compartido, Subscription propia de cada instancia), `sql_server_fqdn` / `sql_database_name` (de `data-sql`), `appinsights_connection_string` (de `observability`), `apim_service_tag` (default `ApiManagement`, para `ip_restriction`).
- `servicebus_topic_name`/`servicebus_subscription_name`, `sql_server_fqdn` y `sql_database_name` no controlan ningún recurso de este módulo — solo viajan como app settings (`ServiceBusTopicName`, `ServiceBusSubscriptionName`, `SqlServer__Fqdn`, `SqlServer__Database`) para que el código de `ValidatePayment`/`ProcessPayment` (repo aparte, [`novapay-functions`](https://github.com/plazasgiovanny/novapay-functions)) no hardcodee nombres de recursos por ambiente.
...
- **`ip_restriction`** (`site_config`, default `Deny` + regla que permite el `service_tag` de APIM): el Function App solo acepta tráfico del gateway, nunca directo de Internet — la function key es defensa en profundidad, no la única barrera. Deliberadamente no toca `scm_ip_restriction`: el pipeline de despliegue de código (`novapay-functions`) usa ese endpoint desde runners de GitHub Actions con IPs dinámicas.

Depende de `networking` (subred delegada), `messaging-servicebus` (Topic/Subscription) y `observability` (Application Insights) — capa 2: plataforma de ejecución, mismo nivel que `compute-appservice`.
