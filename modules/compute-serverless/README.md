...
- `integracion_subnet_id` (de `networking`, subred delegada a `Microsoft.Web/serverFarms`), `max_instance_count` (sin default — obliga a decidir explícitamente por ambiente, ver más abajo), `servicebus_namespace_fqdn` / `servicebus_queue_name` (de `messaging-servicebus`), `sql_server_fqdn` / `sql_database_name` (de `data-sql`), `appinsights_connection_string` (de `observability`).
- `servicebus_queue_name`, `sql_server_fqdn` y `sql_database_name` no controlan ningún recurso de este módulo — solo viajan como app settings (`ServiceBusQueueName`, `SqlServer__Fqdn`, `SqlServer__Database`) para que el código de `ValidatePayment`/`ProcessPayment` (repo aparte, `functions/NovaPay.Payments`) no hardcodee nombres de recursos por ambiente.
