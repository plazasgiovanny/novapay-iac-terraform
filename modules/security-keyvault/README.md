...
Capa 1 (red y seguridad base): depende de `networking` para el Private Endpoint. Las asignaciones de rol "Key Vault Secrets User" hacia las identidades de `compute-appservice` (mínimo privilegio, sección 4.3) se declaran en la composición raíz, no en este módulo, para evitar una dependencia circular entre ambos (ver comentario en `main.tf`).
